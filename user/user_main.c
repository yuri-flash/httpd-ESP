...
HttpdBuiltInUrl builtInUrls[]={
	{"*", cgiRedirectApClientToHostname, "pointsense"},
	//{"*", cgiRedirectApClientToHostname, "esp8266.nonet"},
	{"/", cgiRedirect, "/index.html"},
	{"/led.tpl", cgiEspFsTemplate, tplLed},
	{"/index.tpl", cgiEspFsTemplate, tplCounter},
	{"/led.cgi", cgiLed, NULL},
	{"/time.cgi", cgiTime, NULL},
...
void CbRXSerial(uint8 CharBuffer)
{
	...
	//Send the character
	WRITE_PERI_REG(UART_FIFO(0), CharBuffer);

	temp_buffer[temp_offset] = CharBuffer;

	if (CharBuffer == '\r'){
		temp_buffer[temp_offset+1] = '\0';
		os_sprintf(time_buffer_str,temp_buffer);
		os_printf(time_buffer_str);
		temp_offset = 0;
	}else{
